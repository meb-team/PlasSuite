...
for contig in dic_gff: 
	...
	o2.write(contig_name+"\n") 
	if (len(contig)>1 and int(contig[-1].end) >= int(contig_length) and int(contig[0].end)==int(contig[-1].end)-int(contig_length)): 
		contig=contig[1:]
	for c in contig: 
